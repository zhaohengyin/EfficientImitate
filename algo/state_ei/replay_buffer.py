...
def get_expert_buffer(path, lim=100000):
    buffer = {}
...
class ReplayBuffer:
    ...
    def __init__(self, initial_checkpoint, initial_buffer, config, test_throughput=False):
        ...
        if hasattr(config, 'nd'):
            lim = config.nd
        else:
            lim = 100000

        self.expert_buffer = get_expert_buffer(config.expert_demo_path, lim=lim)
    ...
    def sample_position(self, game_history, force_uniform=False):
        ...
        if self.config.PER and not force_uniform:
            ...
            position_index = numpy.random.choice(len(game_history.root_values) - self.config.td_steps)
    ...
    def sample_n_expert_games(self, n_games):
        ...
        for game_id in selected_games:
            ...
            game_pos = numpy.random.choice(len(game.observation_history))
            pos_prob = 1 / len(game.observation_history)

            ret.append((game_id, game.subset(max([0, game_pos - self.config.stacked_observations + 1]), 16),
                        1, game_pos, pos_prob))
