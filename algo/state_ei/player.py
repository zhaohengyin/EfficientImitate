...
import time
import torch
from algo.state_ei.mcts import MCTS
...
class Player:
    def __init__(self, config, seed, n_parallel):
        ...
        self.eval_model = MLPModel(config)
    ...
    def run_eval(self, trained_steps=0):
        ...
        while sum(dones) < 10:
            steps += 1
            root_values, root_distributions, root_actions = run_multi_gail_support(observations, self.model,
                                                                                   self.config)
    ...
    def run_render(self):
        ...
        all_frames = [[] for _ in range(self.n_parallel)]
        ...
        while sum(dones) < self.n_parallel:
            ...
            for i, game_history in enumerate(game_histories):
                ...
                all_frames[i].append(self.envs[i].env.render(mode='rgb_array',
                                        height=100,
                                        width=100,
                                        camera_id=0))
