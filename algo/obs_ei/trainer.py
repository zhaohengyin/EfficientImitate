...
from algo.obs_ei.player import Player
import torch_utils
...
class Trainer:
    def __init__(self, checkpoint, config):
        self.model = MuZeroResidualNetwork(config).to(torch.device('cuda'))
        self.model.set_weights(copy.deepcopy(checkpoint["weights"]))
        self.model.train()

        self.target_model = MuZeroResidualNetwork(config).to(torch.device('cuda'))
    ...
    def loss_consistency_atomic_fn(self, h, h_t):
        ...
        h_t_flat = h_t.reshape(h_t.size(0), -1).detach()
        h_flat = F.normalize(h_flat, dim=-1, eps=1e-5)
        h_t_flat = F.normalize(h_t_flat, dim=-1, eps=1e-5)
        loss_h = -(h_flat * h_t_flat).sum(dim=1, keepdim=True)
    ...
    def loss_pi_kl_fn(self, policy, target_action, target_policy):
        ...
        distr = SquashedNormal(policy[:, :action_dim], policy[:, action_dim:].exp() + 5e-3)
    ...
    def loss_rc_fn(self, r, target_r):
        loss = torch.sigmoid(target_r.detach()) * F.logsigmoid(r + 1e-4) + \
               (1 - torch.sigmoid(target_r.detach())) * F.logsigmoid(1 - r + 1e-4)
    ...
    def continuous_update_weights(self, batch_buffer, replay_buffer, target_workers, shared_storage):
        ...
        while self.training_step < self.config.training_steps and not ray.get(
            ...
            batch_pop = batch_buffer.pop()
            if batch_pop is None or batch_pop == -1:
                time.sleep(0.3)
                continue

            batch_ = copy.deepcopy(batch_pop)
            del batch_pop
            gc.collect()

            index_batch, batch_step, batch = batch_
            ...
            print("Training Step:{}; Batch Step:{}; P:{:.3f}, V:{:.3f}, R:{:.3f}, FRP:{:.3f}, FRE:{:.3f}, C:{:.3f}, "
                  "BC:{:.3f}, E:{:.3f}, Hidden:{:.3f}, G:{:.3f} | GMax:{:.3f}, GMean:{:.3f}, GMin:{:.3f} | BVMax:{:.3f}, BVMean:{:.3f}, "
                  "BVMin:{:.3f} | Time:{:.3f}".format(
                self.training_step, batch_step, policy_loss, value_loss, reward_loss, first_step_reward_loss_pi,
                first_step_reward_loss_exp,
                consistency_loss, entropy_loss, consistency_hidden_loss, bc_loss, grad_loss,
            ...
            if self.config.ratio_lower_bound:
                ...
                if (
                    num_played_games < desired_played_games
            ...
            # Logs.
            def write_sequential(info, key):
                vals = info[key]
                for i in range(len(vals)):
                    writer.add_scalar("Values/"+key+'_'+str(i), vals[i], self.training_step)
            ...
            if self.training_step % 10 == 0:
                ...
                writer.add_scalar("Training/Hidden_loss", consistency_hidden_loss, self.training_step)
                writer.add_histogram("Gail_Reward", gail_r, self.training_step)
                writer.add_histogram("Boostrap_V", bootstrap_v, self.training_step)
                write_sequential(debug_info, "hidden_means")
                write_sequential(debug_info, "pi_losses")
                write_sequential(debug_info, "exp_losses")
    ...
    def update_weights(self, batch):
        ...
        for i in range(1, self.config.num_unroll_steps_reanalyze + 1):
            ...
            # TODO(): This part should be modified accordingly.
            # We add gradient penalty here.
            gp_hidden_pi = hidden_state[:batchsize // 2, :].detach()  # [Batchsize, hidden_c, hidden_h, hidden_w]
            gp_action_pi = action_batch[:batchsize // 2, i]
            gp_hidden_exp = hidden_state[batchsize // 2:, :].detach()  # [Batchsize, hidden_c, hidden_h, hidden_w]
            ...
            interpolate_hidden = autograd.Variable(interpolate_hidden, requires_grad=True)
            interpolate_action = autograd.Variable(interpolate_action, requires_grad=True)

            _, output_interpolate = self.model.dynamics(interpolate_hidden, interpolate_action)

            gradients_hidden, gradients_action = autograd.grad(outputs=output_interpolate,
                                                               inputs=[interpolate_hidden, interpolate_action],
                                                               grad_outputs=torch.ones(
                                                                   output_interpolate.size()).cuda(),
                                                               create_graph=True, retain_graph=True, only_inputs=True)

            gradients = torch.cat((gradients_hidden.reshape(batchsize // 2, -1),
                                   gradients_action.reshape(batchsize // 2, -1)), dim=-1)
            ...
            target_hidden = self.model.representation(self.transform.transform(next_observation_batch[:, i]))
            step_consistency_loss, step_consistency_hidden_loss = self.loss_consistency_atomic_fn(hidden_state,
                                                                                              target_hidden)

            bc_loss += self.loss_bc(policy_info_bc[batchsize // 2:], action_batch[batchsize // 2:, i + 1])
            consistency_loss += (step_consistency_loss * mask_batch[:, i:(i + 1)]).squeeze()
            consistency_hidden_loss += (step_consistency_hidden_loss * mask_batch[:, i:(i + 1)]).squeeze()

            reward_step_loss = self.loss_reward_fn(r_pi_logits=reward[:batchsize//2, :],
                                                   r_exp_logits=reward[batchsize//2:, :])

            all_l2_means.append(hidden_state.mean().item())
            all_pi_vals.append(reward_step_loss[:batchsize//2].mean().item())
            all_exp_vals.append(reward_step_loss[batchsize//2:].mean().item())
        ...
        return (
            ...
            consistency_hidden_loss.mean().item(),
            policy_entropy_loss.mean().item(),
            gradient_penalty_loss.mean().item(),
            {
                'hidden_means': all_l2_means,
                'pi_losses': all_pi_vals,
                'exp_losses': all_exp_vals,
            },
            bc_loss.mean().item()
        )
