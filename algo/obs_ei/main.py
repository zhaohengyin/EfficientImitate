...
from algo.obs_ei.trainer import Trainer
from algo.obs_ei.replay_buffer import ReplayBuffer
from algo.obs_ei.player import PlayerWorker
from algo.obs_ei.batch_worker import BatchWorker, BatchBuffer
from algo.obs_ei.parallel_model import RZeroMLPWorker
...
class RZero:
    ...
    def train_new(self, log_in_tensorboard):
        ...
        # Initialize workers
        # Model workers are used by the selfplay workers.
        self.model_workers = [
            RZeroMLPWorker.remote(self.config)
            for _ in range(int(math.ceil(self.config.num_workers / self.config.selfplay_model_serve_num)))
        ]
        ...
        # Worker: Batch Buffer
        self.batch_buffer_worker = BatchBuffer()
...
class CPUActor:
    ...
    def get_initial_weights(self, config):
        model = MuZeroResidualNetwork(config)
