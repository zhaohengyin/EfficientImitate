...
from algo.obs_ei.trainer import MuZeroResidualNetwork
from mcts_tree_sample.mcts import run_multi_gail_support_fast
import gc
...
class BatchWorker:
    ...
    def calculate_bootstrap(self, observations, masks):
        ...
        root_values, root_distributions, root_actions = run_multi_gail_support_fast(observations,
    ...
    def get_batch(self):
        ...
        for i in range(self.config.num_unroll_steps_reanalyze + 1):
            step_bootstrap_values = []
            step_raw_actions_batch = []
            step_raw_policies_batch = []
            step_target_mu_batch = []
            step_target_std_batch = []

            for j in range(self.config.batch_size // self.config.re_batchsize):
                sub_game_samples = game_samples[self.config.re_batchsize * j: self.config.re_batchsize * (j+1)]
                ...
                for (game_id, game_history, game_prob, game_pos, pos_prob) in sub_game_samples:
                ...
                step_bootstrap_values.append(bootstrap_values)
                ...
                step_raw_actions_batch.append(begin_actions)
                step_raw_policies_batch.append(begin_policies)

                policy_mu, policy_std = calculate_mean_std(begin_actions, begin_policies)
                step_target_mu_batch.append(policy_mu)
                step_target_std_batch.append(policy_std)

            # Gather the results.
            step_bootstrap_values = np.concatenate(step_bootstrap_values, axis=0)
            step_raw_policies_batch = np.concatenate(step_raw_policies_batch, axis=0)
            step_raw_actions_batch = np.concatenate(step_raw_actions_batch, axis=0)
            step_target_std_batch = np.concatenate(step_target_std_batch, axis=0)
            step_target_mu_batch = np.concatenate(step_target_mu_batch, axis=0)

            all_bootstrap_values.append(step_bootstrap_values)
            raw_actions_batch.append(step_raw_actions_batch)
            raw_policies_batch.append(step_raw_policies_batch)
            target_mu_batch.append(step_target_mu_batch)
            target_std_batch.append(step_target_std_batch)
    ...
    def make_target(self, game_history, state_index=0, bootstrap_value=None):
        ...
        for current_index in range(
            ...
            if current_index < len(game_history.observation_history) - 1:
                ...
                # Magical number...
                random_actions = self.config.sample_random_actions(8) #len(game_history.child_visits[0]))
